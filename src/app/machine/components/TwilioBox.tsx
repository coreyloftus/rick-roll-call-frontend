...
export default function TwilioBox(audioURL?: string, audioBlob?: Blob) {
    ...
    return (
        <Box className='twilio-box'>
            ...
            <Typography>{audioURL ? `Audio URL: ${audioURL}` : 'No audio URL provided.'}</Typography>
            <Typography>
                {audioBlob ? `Audio Blob Size: ${audioBlob.size} bytes` : 'No audio blob provided.'}
            </Typography>
        </Box>
    )
}
